...
class Game(Scene):
    ...
    def on_draw(self, surface):
        ...
        Texto(self.fonte, f'${self.dinheiro}', (255, 255, 255), [20, 20]).draw(surface)
        
        self.fish.draw(surface)

        if self.preco_melhoria <= self.dinheiro:
            self.melhoria.draw(surface)
            Texto(self.fonte, f'Melhorar: ${self.preco_melhoria}', (255, 255, 255), [160, 90]).draw(surface)
        ...
        # Mouse click events
            ...
            if pygame.mouse.get_pressed()[0]:
                if self.fish.on_event():
                    texto_timer = Texto(self.fonte, self.dinheiro, (240, 240, 20), [20, 20], self.preco_peixe, 400)
                    ...
                    if self.dinheiro >= self.preco_melhoria:
                        texto_timer = Texto(self.fonte, self.dinheiro, (240, 40, 10), [20, 20], -self.preco_melhoria, 400)
                        self.fluxo_moedas.append(texto_timer)

                        self.dinheiro -= self.preco_melhoria
                        self.preco_melhoria *= 2
                        self.preco_peixe *= 2
